...
class Main extends PluginBase implements Listener {
    ...
    public function is_valid_domain_name(string $domain_name): bool{
        return (preg_match("/([a-z\d](-*[a-z\d])*)(\.([a-z\d](-*[a-z\d])*))*:(\d{1,5})/i", $domain_name)) and preg_match("/.{1,253}/", $domain_name) and preg_match("/[^.]{1,63}(\.[^.]{1,63})*/", $domain_name); //valid chars check and preg_match("/.{1,253}/", $domain_name) //overall length check and preg_match("/[^.]{1,63}(\.[^.]{1,63})*/", $domain_name)); //length of each label
    }
